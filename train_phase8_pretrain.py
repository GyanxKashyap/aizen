"""
Phase 8 stage 1: pretrain AIZEN-40M from scratch on the doubled TinyStories
corpus (~200MB, ~52M tokens).

Scale experiment: ONLY the model size changes vs Phase 5
(8L/384d/6h 16M -> 12L/512d/8h ~40M). Same tokenizer v2, same 512 ctx,
same warmup+cosine recipe. Batch 24 (memory headroom at the bigger size).

Run:  python3 train_phase8_pretrain.py [--steps 16000]
      (chunked/resumable via CHUNK_ITERS env)
"""
...
RESUME_PATH = "checkpoints/phase8_pretrain_resume.pt"
FINAL_PATH = "aizen_phase8_pretrained.pt"
METRICS_PATH = "results/metrics_phase8_pretrain.csv"
WARMUP = 300
...
data = torch.load("data/pretrain_tokens_v2.pt")
ids_all = data["ids"]
starts = data["story_starts"]
n_train_stories = int(len(starts) * 0.99)

cfg = {"block_size": BLOCK, "n_embd": args.n_embd, "n_head": args.n_head, "n_layer": args.n_layer}
model = Aizen(vocab_size=tok.vocab_size, **cfg, dropout=0.1).to(DEVICE)
print(f"device: {DEVICE} | {len(ids_all):,} tokens, {len(starts):,} stories")
...
CHUNK_ITERS = int(os.environ.get("CHUNK_ITERS", 150))
...
if end_iter >= args.steps:
    torch.save({"model": model.state_dict(), "config": cfg,
                "tokenizer": tok.state(),
                "meta": {"phase": "phase8_pretrain", "steps": args.steps,
                         "lr": args.lr, "batch_size": args.batch_size,
                         "corpus": "TinyStories 200MB (two slices)", "seed": args.seed}},
               FINAL_PATH)
